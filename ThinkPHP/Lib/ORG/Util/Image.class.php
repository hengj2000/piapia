...
class Image extends Think {//类定义开始
    ...
    static function buildImageVerify($length=4, $mode=1, $type='png', $width=48, $height=22, $verifyName='verify') {
        ...
        $backColor = imagecolorallocate($im, 1, 21, 45);    //背景色（随机）
        $borderColor = imagecolorallocate($im, 10, 48, 57);                    //边框色
        $pointColor = imagecolorallocate($im, mt_rand(0, 255), mt_rand(0, 255), mt_rand(0, 255));                 //点颜色

        @imagefilledrectangle($im, 0, 0, $width - 1, $height - 1, $backColor);
        @imagerectangle($im, 0, 0, $width - 1, $height - 1, $borderColor);
        $stringColor = imagecolorallocate($im, mt_rand(24, 28), mt_rand(156, 158), mt_rand(177, 180));
        // 干扰
        for ($i = 0; $i < 10; $i++) {
            $fontcolor = imagecolorallocate($im, mt_rand(19, 20), mt_rand(42, 44), mt_rand(50, 55));
