...
class PublicAction extends Action {
    ...
    protected function ajaxUploadResult($info) {
        ...
        //alert($info);
        $show = '<script language="JavaScript" src="/Tpl/default/Public/Js/mootools.js"></script><script language="JavaScript" type="text/javascript">' . "\n";
        ...
        if ($info['success']) {
            // 提示上传成功
            $show .= 'result.innerHTML = "<div style=\"color:#3333FF\" id=\"pplabel\"> <a href=\"javascript:delpp('. $info['uploadId'] .')\"'. $info['uploadId'] .'\">重新传</a></div>";';
            // 如果定义了成功响应方法，执行客户端方法
            // 参数为上传的附件id，多个以逗号分割
